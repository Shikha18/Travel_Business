...
export default function Footer() {
  ...
  return (
    <footer className="text-ink-700" style={{ background: "#080806", borderTop: "1px solid rgba(228,170,66,0.12)" }}>
      <div className="container-wide grid gap-10 py-12 md:grid-cols-4">
        <div className="md:col-span-2">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {siteConfig.trustBadges.map((badge) => (
              <span
                key={badge}
                className="inline-flex items-center rounded-full border border-white/10 px-3 py-1 text-xs text-ink-700"
              >
                {badge}
              </span>
            ))}
        ...
        <div>
          <h4 className="text-sm font-semibold uppercase tracking-wider text-ink-600">
            {t("footer.contact")}
          </h4>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        <div className="container-wide flex flex-col items-start justify-between gap-2 py-5 text-xs text-ink-600 md:flex-row md:items-center">
          <p>
